...
async function carregarProdutosDoFirestore() {
    ...
    const product = productsData.find(p => p.id === productId);
                ...
                document.getElementById('productId').value = productId; // Agora salva o ID do Firestore
...
// Mostrar loading
    ...
    try {
        ...
        saveProducts(); // Salva no localStorage também
        closeProductModal();
        renderAdminProducts();
        renderProducts();
        updateAdminStats();
    ...
    // ADICIONE estas linhas APÓS o catch, ANTES do fechamento da função
        ...
        function updateCartUI() {
            ...
            if (cart.length === 0) {
                ...
                cartItems.innerHTML = cart.map(item => `
                    <div class="cart-item">
                        <div class="cart-item-img" style="background: ${item.image}"></div>
